...
6. Supplemental overlay visibility MUST default to off and MUST be user-toggled (`G`).
7. GSPS overlays MUST attach by SOP Instance UID match only.
8. Mammography CAD SR overlays MUST attach by direct referenced-image SOP Instance UID match only, and only `Presentation Required` findings participate in render/navigation.
9. `open_group` MUST be validated/clamped before use.
10. Streaming completion logic MUST compare image counts (not total paths including GSPS/SR).
11. UI state mutations MUST stay on the main thread; workers MUST communicate through channels.
12. Production diagnostics MUST use logging (`log` macros), not `println!/eprintln!`.
13. DICOMweb metadata parsing MUST use top-level instance identifiers; nested reference tags inside GSPS/SR sequences MUST NOT override the owning series or instance identity.
14. Grouped DICOMweb launch MUST resolve and stream the `open_group` before background groups so first-image latency is driven by the active group only.
15. Background DICOMweb groups MUST stage into history as each group download completes; history thumbnails and group switching MUST NOT wait for the final grouped download result.
16. If the user switches away from a streaming DICOMweb active group, remaining active-group work MUST continue staging into history and MUST NOT clear, replace, or visually mask the currently displayed study.
17. Multi-frame images with per-frame `ImagePositionPatient` MUST expose frames in logical patient-position order; if the dominant per-frame patient-position progression increases across stored frames, display and cine MUST reverse with it, and GSPS/SR frame lookups MUST translate the displayed frame back to the referenced stored DICOM frame.
18. DICOM content inside the viewer MUST use explicit `DicomSource` ownership; DICOMweb bytes MUST be represented as `DicomSource::Memory`, not temp files or a global backing store.
